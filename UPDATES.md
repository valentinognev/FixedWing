...
- Live GZ 120 s `race_quat` `pn` 2026-09-04 07:12 `/tmp/balloon_race_race_quat_gz_baseline.csv` (+ `.pkl`): first circuit B0 **3.73** (XY 0.69, ΔD −3.66, t=47.3) / B1 **0.57** (XY 0.07, ΔD −0.57, t=63.9) / B2 **3.61** (XY 1.78, ΔD −3.14, t=77.5) m. Gate ≤5 m passed. Residual is systematically **high** (NED +z down; negative ΔD = above the balloon): descending B0/B2 ~3.2–3.7 m high; climb B1 is the accurate one. Pickle 20 Hz vz sign-flips **66** (`FlightHistory.from_pickle` `vz` interpolated onto a 0.05 s grid) vs `race_euler` winner 21.
