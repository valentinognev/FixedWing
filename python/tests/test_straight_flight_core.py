#!/usr/bin/env python3
"""Source contracts for path settle (JSBSim race has no fly-by)."""
...
class TestJsbsimPathSettleAndFlyby(unittest.TestCase):
    ...
    def test_balloon_control_jsbsim_path_settle_and_no_flyby(self) -> None:
        ctl = _CTL.read_text(encoding="utf-8")
        self.assertIn("settle_path_altitude", ctl)
        self.assertNotIn("flyby_radius_from_speed", ctl)
        self.assertNotIn("coordinated_turn_radius_m", ctl)
        self.assertNotIn("turn_radius_m", ctl)
