#!/usr/bin/env python3
"""
Start headless JSBSim Rascal SITL (optional) and immediately hold straight flight
via OFFBOARD LOCAL_NED position+velocity at 1.5 * Vstall (Rascal ≈ 10 kt).

PX4 fixed-wing OFFBOARD only runs guidance when position control is enabled.
Velocity-only setpoints fall into FW_POSCTRL_MODE_OTHER (no TECS/path). We send
a waypoint ahead on a locked course at a fixed NED altitude (Z hold), plus
course velocity so AUTO_PATH holds altitude and tracks the bearing.

Usage:
  python3 python/run_straight_flight_headless.py
  python3 python/run_straight_flight_headless.py --no-sim --duration=120
  python3 python/run_straight_flight_headless.py --course-deg=0   # due north
  python3 python/run_straight_flight_headless.py --no-plot        # skip history window
"""
...
DEFAULT_SPEED_MPS = 1.5 * RASCAL_V_STALL_MPS  # ≈ 7.72 m/s
...
def read_local_position(
    ...
    while time.time() < deadline:
        ...
        # Reject the common pre-estimate (0,0,0) packet.
...
def engage_offboard_asap(
    ...
    """Stream setpoints, switch OFFBOARD, and arm with minimal wall-clock delay."""
    period = 1.0 / max(rate, 1.0)
    ...
    print("Arming (force immediately for in-air SITL)...")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "OFFBOARD LOCAL_NED straight flight for headless JSBSim Rascal SITL "
            f"(default speed 1.5*Vstall ≈ {DEFAULT_SPEED_MPS:.2f} m/s; "
            "ahead waypoint + locked altitude)"
        )
    )
    parser.add_argument(
        "--no-sim",
    ...
    parser.add_argument(
        ...
        help=(
            f"Horizontal speed m/s / airspeed target "
            f"(default: 1.5*Vstall={DEFAULT_SPEED_MPS:.2f})"
        ),
    )
    parser.add_argument(
        "--vstall",
        type=float,
        default=RASCAL_V_STALL_MPS,
        help=f"Stall speed m/s used when deriving default (default Rascal {RASCAL_V_STALL_MPS:.2f})",
    )
    parser.add_argument(
        "--course-deg",
        type=float,
    ...
    if abs(args.vstall - RASCAL_V_STALL_MPS) > 1e-6 and abs(
        args.speed - DEFAULT_SPEED_MPS
    ) < 1e-6:
    ...
    if args.course_deg is not None:
        ...
        print(f"Locking course to heading at engage: {math.degrees(course_rad) % 360.0:.1f}°")

    x0, y0, z_hold = read_local_position(master)
    xy = [x0, y0]
    vx, vy, vz = ned_velocity_from_course(speed, course_rad)
    print(
        f"Altitude hold z_ned={z_hold:.1f} m; waypoint {lookahead_m:.0f} m ahead on course"
    )
    print(
        f"Engage ASAP: LOCAL_NED vel=({vx:.2f},{vy:.2f},{vz:.2f}) m/s "
        f"|v|={speed:.2f} (1.5*Vstall, Vstall={args.vstall:.2f}) at {args.rate} Hz"
    )
    ...
    while not stop_requested:
        ...
        if got is not None:
            xy[0], xy[1] = got[0], got[1]
            # If engage used a placeholder origin, lock altitude on first real sample.
    ...
    _stop_sim()
    if not args.no_plot:
        history.plot(title="Headless JSBSim straight flight")
    return 0
